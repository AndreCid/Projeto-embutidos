	...
	ArPathPlanningInterface *pathTask, ArFunctor *shutdownFunctor)
{
  ...
  {
    ArLog::log(ArLog::Normal, "ArServerModeDock::createDock: Robot is not connected, cannot create docking mode");
    return NULL;
  }
  ...
  if ((origConfig = robot->getOrigRobotConfig()) != NULL)
  {
    ...
    else if (origConfig->getHasCharger() == 6)
    {
...
void ArServerModeDock::dockUserTask(void)
{
...
bool ArServerModeDock::processFile(void)
{
  ...
  myGoalFailedCB(this, &ArServerModeDockTriangleBumpBackwards::goalFailed)
{
  ...
  myDrivingInAvoidanceAbsoluteLengthFront = 548;
...
AREXPORT void ArServerModeDockLynx::restoreFromDockFile(void)
{
  ...
  // MPL 2014_04_16 changed it so we don't just believe the charging
  // state, but instead rely on the flags from the battery currently
...
AREXPORT void ArServerModeDockLynx::checkDock(void)
{
  ...
  /* MPL 2014_04_16 changing this so that we don't believe anything
   * but on_charger anymore, since everything else seems to be
   * mistaken sometimes

  bool isDockedNow = isDocked();

  ArLog::log(ArLog::Normal, 
       "Docking::checkDock: isDockedNow %d chargeState %d statusFlags 0x%04x", 
	     isDockedNow, myRobot->getChargeState(), statusFlags);
  */

  ArLog::log(ArLog::Normal, "Docking::checkDock: statusFlags 0x%04x", 
	     statusFlags);
  if (//isDockedNow || 
      //(statusFlags & ArBatteryMTX::STATUS_CHARGER_ON) || 
	  (statusFlags & ArBatteryMTX::STATUS_ON_CHARGER))
  {
    /*
    if (isDockedNow)
      ArLog::log(ArLog::Normal, "Docking::checkDock: Robot is charging, activating dock mode");
    else if (statusFlags & ArBatteryMTX::STATUS_CHARGER_ON) 
      ArLog::log(ArLog::Normal, "Docking::checkDock: Robot charger is on, activating dock mode");
    else 
    */
    if (statusFlags & ArBatteryMTX::STATUS_ON_CHARGER) 
      ArLog::log(ArLog::Normal, "Docking::checkDock: Robot is on charger, activating dock mode");
    else
      ArLog::log(ArLog::Normal, "Docking::checkDock: For unknown reason, activating dock mode");
    myStatus = "Docked";
    switchState(DOCKED);
    myHitDock = true;
    activate();
  ...
  {
    ArLog::log(ArLog::Normal, "Docking::checkDock: Not docked, doing nothing");
  }
}
